...
TEST(treeTest, setData_getData) {
  s21::SelfBalancingBinarySearchTree a;
...
TEST(treeTest, setData_getData2) {
  s21::SelfBalancingBinarySearchTree a;
  std::string key1 = "student1";
  s21::data value1 = {"Ivanov", "Nikolay", 1995, "Syktyvkar", 101, 1};
  a.SetData(key1, value1);
  std::this_thread::sleep_for(std::chrono::seconds{1});
  a.DeleteOldNode();
  s21::data value2 = {"Ivanov", "Nikolay", 1995, "Syktyvkar", 101};
  a.SetData(key1, value2);
  ASSERT_EQ(a.GetData(key1).year_of_birth, 1995);
  a.Clear();
}

TEST(treeTest, renameKeys) {
  s21::SelfBalancingBinarySearchTree a;
  std::string key1 = "student1";
...
TEST(treeTest, renameKeys2) {
  ...
  std::this_thread::sleep_for(std::chrono::seconds{1});
  a.DeleteOldNode();
  EXPECT_ANY_THROW(a.RenameKeys(key1, key2));
  a.Clear();
}

TEST(treeTest, exists) {
  s21::SelfBalancingBinarySearchTree a;
  std::string key1 = "student1";
  std::string key2 = "engineer1";
  s21::data value1 = {"Ivanov", "Nikolay", 1995, "Syktyvkar", 101};
...
TEST(treeTest, getTTL) {
  ...
  a.DeleteOldNode();
  EXPECT_ANY_THROW(a.GetTTL(key1));
  a.Clear();
}

TEST(treeTest, automaticClear) {
  s21::SelfBalancingBinarySearchTree a;
  for (int i = 0; i < 1000000; ++i) {
    std::string ii = std::to_string(i);
    std::string key = "key" + ii;
    s21::data value = {"Ivanov" + ii, "Nikolay", 1995, ii, i, 2};
    a.SetData(key, value);
  }
  std::this_thread::sleep_for(std::chrono::seconds{2});
  a.DeleteOldNode();
  ...
  std::vector<std::string> rez = a.ReturnKeys();
  ASSERT_EQ(rez.size(), 4);
  ASSERT_EQ(a.GetData("key1000001").surname, "Ivanov1000001");
  ASSERT_ANY_THROW(a.GetData("key0"));
  ASSERT_ANY_THROW(a.GetData("key999999"));
  a.Clear();
}

TEST(treeTest, findFull) {
  s21::SelfBalancingBinarySearchTree a;
  std::vector<std::string> key = {"student1", "student2", "engineer1",
                                  "engineer2"};
  s21::data value1 = {"Ivanov", "Nikolay", 1995, "Syktyvkar", 101};
...
TEST(treeTest, Delete50000_51000) {
  s21::SelfBalancingBinarySearchTree a;
  a.LoadData("treeload.txt");
  std::random_device rd;
  std::default_random_engine eng(rd());
